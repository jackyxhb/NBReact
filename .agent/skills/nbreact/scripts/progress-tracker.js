...
function calculateOverallAverage(history) {
  const total = Object.values(history.topicStats).reduce((sum, stats) => sum + stats.totalScore, 0);
  return history.totalQuestions > 0 ? Math.round(total / history.totalQuestions) : 0;
}
